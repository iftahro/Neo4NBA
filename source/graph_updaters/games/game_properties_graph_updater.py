...
MATCH (g:Games)
WITH g, g.wins + '-' + g.losses AS record
SET g.record = record
"""
...
ADD_ROSTER_HOME_AWAY_STATS = """
MATCH (r:Roster)-[:PLAYED]->(g:Games:{game_label})
...
game_properties_graph_updater = GraphUpdater("game_properties_graph_updater", [
    ...
    ADD_GAMES_FINAL_SCORE,
    ADD_GAMES_WIN_PERCENTAGE,
    ADD_ROSTER_HOME_AWAY_STATS.format(game_label='Away', game_type='away'),
    ADD_ROSTER_HOME_AWAY_STATS.format(game_label='Home', game_type='home'),
    ADD_ROSTER_CHARACTER.format(prop='home_win%', min='0.85', character='Watchdogs'),
    ADD_ROSTER_CHARACTER.format(prop='away_win%', min='0.7', character='RoadHunters')
])
